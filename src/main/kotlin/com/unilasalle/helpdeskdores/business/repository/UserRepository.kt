...
class UserRepository {
    ...
    fun findByEmail(email: String): User? {
        val firestoreDb = firebaseInitializer.getFirebase()
        val documentReference = firestoreDb?.collection(USER_COLLECTION_NAME)?.document(email)
        val documentSnapshot: ApiFuture<DocumentSnapshot> = documentReference?.get() as ApiFuture<DocumentSnapshot>
        val document: DocumentSnapshot = documentSnapshot.get()
    ...
    fun emailAvailable(value: String): Boolean {
        val firestoreDb = firebaseInitializer.getFirebase()
        val documentReference = firestoreDb?.collection(USER_COLLECTION_NAME)?.document(value)
        val documentSnapshot: ApiFuture<DocumentSnapshot> = documentReference?.get() as ApiFuture<DocumentSnapshot>
        val document: DocumentSnapshot = documentSnapshot.get()
